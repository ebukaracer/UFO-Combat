    ...
    public class Pool : MonoBehaviour
    {
        ...
        */
        ...
        private void InsertObjectToQueue()
        {
            ...
            if (poolObj == null)
            {
                Logging.LogWarning($"No <b>{nameof(PoolObject)} Component</b> was added or inherited <i>{poolObjectPrefab.name}</i> gameobject." +
        ...
        public PoolObject SpawnObject(Vector3 position, Quaternion rotation)
        {
            if (_queue.Count == 0)
            {
                ...
                {
                    Logging.LogError($"[<color=#ff5566><b>Pool out of objects error</b></color>] : No more game objects available in the <i>{name}</i> pool.\n"
